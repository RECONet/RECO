...
//------------------------------------------------------------------------------
int mme_app_send_s11_release_access_bearers_req (ue_mm_context_t *const ue_mm_context, const pdn_cid_t pdn_index)
{
  OAILOG_FUNC_IN (LOG_MME_APP);
  /*
...
int mme_app_send_s11_create_session_req (ue_mm_context_t *const ue_mm_context, const pdn_cid_t pdn_cid)
{
  ...
  bearer_context_t *bc = ue_mm_context->GetBearerContext(ue_mm_context->pdn_contexts[pdn_cid]->default_ebi);
  ...
  session_request_p->sender_fteid_for_cp.teid = reinterpret_cast<uintptr_t>(ue_mm_context);;    // may lead to bug!
  ...
  mme_app_desc.UpdateCollKeys(ue_mm_context,
  ...
  apn_configuration_t *selected_apn_config_p = ue_mm_context->GetApnConfig(ue_mm_context->pdn_contexts[pdn_cid]->context_identifier);
  ...
  session_request_p->paa.pdn_type = static_cast<pdn_type_value_t>(selected_apn_config_p->pdn_type);
