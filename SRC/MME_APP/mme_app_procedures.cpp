...
mme_app_s11_proc_create_bearer_t* mme_app_create_s11_procedure_create_bearer(ue_mm_context_t * const ue_context_p)
{
  mme_app_s11_proc_create_bearer_t *s11_proc_create_bearer =
          static_cast<mme_app_s11_proc_create_bearer_t *>(calloc(1, sizeof(mme_app_s11_proc_create_bearer_t)));
  ...
  if (!ue_context_p->s11_procedures) {
    ue_context_p->s11_procedures =
            static_cast<struct s11_procedures_s *>(calloc(1, sizeof(struct s11_procedures_s)));
    LIST_INIT(ue_context_p->s11_procedures);
  }
...
void mme_app_s11_procedure_create_bearer_send_response(ue_mm_context_t * const ue_context_p, mme_app_s11_proc_create_bearer_t* s11_proc_create)
{
  ...
  for (int ebix = 0; ebix < BEARERS_PER_UE; ebix++) {
    ...
    if (S11_PROC_BEARER_FAILED == s11_proc_create->bearer_status[ebix]) {
      bearer_context_t * bc = ue_context_p->GetBearerContext(ebi);
