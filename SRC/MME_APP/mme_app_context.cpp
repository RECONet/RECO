...
#include "esm_ebr.h"
//------------------------------------------------------------------------------
bool if_guti_valid (const guti_t *guti_p) {
  if (guti_p)
    return (0 != guti_p->gummei.mme_code)        ||
           (0 != guti_p->gummei.mme_gid)         ||
           (0 != guti_p->m_tmsi)                 ||
           (0 != guti_p->gummei.plmn.mcc_digit1) ||     // MCC 000 does not exist in ITU table
           (0 != guti_p->gummei.plmn.mcc_digit2) ||
           (0 != guti_p->gummei.plmn.mcc_digit3);
  return false;
}
int guti_cmp (const guti_t *lhs, const guti_t *rhs) {
  if (lhs && rhs)
    return (lhs->gummei.mme_code        != rhs->gummei.mme_code)        ||
           (lhs->gummei.mme_gid         != rhs->gummei.mme_gid)         ||
           (lhs->m_tmsi                 != rhs->m_tmsi)                 ||
           (lhs->gummei.plmn.mcc_digit1 != rhs->gummei.plmn.mcc_digit1) ||     // MCC 000 does not exist in ITU table
           (lhs->gummei.plmn.mcc_digit2 != rhs->gummei.plmn.mcc_digit2) ||
           (lhs->gummei.plmn.mcc_digit3 != rhs->gummei.plmn.mcc_digit3);
  return -1;
}

//------------------------------------------------------------------------------
ue_mm_context_t                           *
mme_ue_context_exists_enb_ue_s1ap_id (MmeUeContextManager * const mme_ue_context_p,
                                      const enb_s1ap_id_key_t enb_key) {
  return mme_ue_context_p->GetUeContextByEnbUeS1apId(enb_key);
}
ue_mm_context_t* MmeUeContextManager::GetUeContextByEnbUeS1apId(const enb_s1ap_id_key_t enb_key) {
  ue_mm_context_t                    *ue_context_p = NULL;

  hashtable_ts_get (enb_ue_s1ap_id_ue_context_htbl, (const hash_key_t)enb_key, (void **)&ue_context_p);
  if (ue_context_p) {
...
//------------------------------------------------------------------------------
ue_mm_context_t                           *
mme_ue_context_exists_mme_ue_s1ap_id (MmeUeContextManager * const mme_ue_context_p,
                                      const mme_ue_s1ap_id_t mme_ue_s1ap_id) {
  return mme_ue_context_p->GetUeContextByMmeUeS1apId(mme_ue_s1ap_id);
}
ue_mm_context_t* MmeUeContextManager::GetUeContextByMmeUeS1apId(const mme_ue_s1ap_id_t mme_ue_s1ap_id) {
  void           *id  = NULL;
  hashtable_rc_t h_rc =
          hashtable_ts_get (mme_ue_s1ap_id_ue_context_htbl, (const hash_key_t)mme_ue_s1ap_id, (void **)&id);

  if (HASH_TABLE_OK == h_rc)
    return GetUeContextByEnbUeS1apId((enb_s1ap_id_key_t)(uintptr_t) id);
  return NULL;
}
//------------------------------------------------------------------------------
ue_mm_context_t* mme_ue_context_exists_imsi (MmeUeContextManager * const mme_ue_context_p,
                                             const imsi64_t imsi) {
  return mme_ue_context_p->GetUeContextByImsi(imsi);
}
ue_mm_context_t* MmeUeContextManager::GetUeContextByImsi(const imsi64_t imsi) {
  void           *id  = NULL;
  hashtable_rc_t h_rc =
          hashtable_ts_get (imsi_ue_context_htbl, (const hash_key_t)imsi, (void **)&id);

  if (HASH_TABLE_OK == h_rc)
    return GetUeContextByMmeUeS1apId((mme_ue_s1ap_id_t)(uintptr_t) id);
  return NULL;
}
//------------------------------------------------------------------------------
ue_mm_context_t                    *
mme_ue_context_exists_s11_teid ( MmeUeContextManager * const mme_ue_context_p,
                                 const s11_teid_t teid) {
  return mme_ue_context_p->GetUeContextByS11Teid(teid);
}
ue_mm_context_t* MmeUeContextManager::GetUeContextByS11Teid(const s11_teid_t s11_teid) {
  void           *id  = NULL;
  hashtable_rc_t h_rc =
          hashtable_ts_get (tun11_ue_context_htbl, (const hash_key_t)s11_teid, (void **)&id);
...
//------------------------------------------------------------------------------
ue_mm_context_t                           *
mme_ue_context_exists_guti (MmeUeContextManager * const mme_ue_context_p,
                            const guti_t * const guti_p) {
  return mme_ue_context_p->GetUeContextByGuti(guti_p);
}
ue_mm_context_t* MmeUeContextManager::GetUeContextByGuti(const guti_t * const guti_p) {
  void           *id  = NULL;
  hashtable_rc_t h_rc =
          obj_hashtable_ts_get (guti_ue_context_htbl, (const void *)guti_p, sizeof (*guti_p), (void **)&id);
  ...
  const bool              is_remove_old)
{
  ...
  h_rc = hashtable_ts_get (mme_app_desc.mme_ue_contexts.mme_ue_s1ap_id_ue_context_htbl, (const hash_key_t)old_mme_ue_s1ap_id,  (void **)&id);
  if (HASH_TABLE_OK == h_rc) {
    ...
    if (old_enb_key != new_enb_key) {
      if (is_remove_old) {
        ...
        if (HASH_TABLE_OK == h_rc) {
          ue_mm_context_t                           *ue_mm_context_new = NULL;
          h_rc = hashtable_ts_get (mme_app_desc.mme_ue_contexts.enb_ue_s1ap_id_ue_context_htbl, (const hash_key_t)new_enb_key, (void **)&ue_mm_context_new);
          old->MoveContentTo(ue_mm_context_new);
          ue_mm_context_new->enb_s1ap_id_key = new_enb_key;
          ue_mm_context_new->enb_ue_s1ap_id  = new_enb_ue_s1ap_id;
          ue_mm_context_new->mme_ue_s1ap_id  = old_mme_ue_s1ap_id;
          ...
          OAILOG_FUNC_RETURN(LOG_MME_APP, ue_mm_context_new);
        }
      } else { // remove new context
        ue_mm_context_t                           *ue_mm_context_new = NULL;
        h_rc = hashtable_ts_remove (mme_app_desc.mme_ue_contexts.mme_ue_s1ap_id_ue_context_htbl, (const hash_key_t)old_mme_ue_s1ap_id, (void **)&id);
        h_rc = hashtable_ts_insert (mme_app_desc.mme_ue_contexts.mme_ue_s1ap_id_ue_context_htbl, (const hash_key_t)old_mme_ue_s1ap_id, (void *)(uintptr_t)new_enb_key);
        h_rc = hashtable_ts_remove (mme_app_desc.mme_ue_contexts.enb_ue_s1ap_id_ue_context_htbl, (const hash_key_t)new_enb_key, (void **)&ue_mm_context_new);
        if (HASH_TABLE_OK != h_rc) {
        ...
        ue_mm_context_new->FreeContent();
        free_wrapper((void**)&ue_mm_context_new);
  ...
  const mme_ue_s1ap_id_t   mme_ue_s1ap_id)
{
  ...
  ue_context_p = mme_app_desc.GetUeContextByEnbUeS1apId(enb_key);
  if (ue_context_p) {
...
{
  mme_ue_context_p->UpdateCollKeys(ue_context_p,
                                   enb_s1ap_id_key, mme_ue_s1ap_id,
                                   imsi, mme_teid_s11, guti_p);
}
void MmeUeContextManager::UpdateCollKeys(
        ue_mm_context_t * const ue_context_p,
        const enb_s1ap_id_key_t enb_s1ap_id_key,
        const mme_ue_s1ap_id_t mme_ue_s1ap_id,
        const imsi64_t imsi,
  ...
  // ToDo: Remove this unnecessary comparison? (all usage of this function pass
  //       ue_context_p->enb_s1ap_id_key as 3rd parameter)
  ...
  if (INVALID_MME_UE_S1AP_ID != mme_ue_s1ap_id) {
    if (ue_context_p->mme_ue_s1ap_id != mme_ue_s1ap_id) {
      // new insertion of mme_ue_s1ap_id, not a change in the id
      h_rc = hashtable_ts_remove (mme_ue_s1ap_id_ue_context_htbl, (const hash_key_t)ue_context_p->mme_ue_s1ap_id,  (void **)&id);
      h_rc = hashtable_ts_insert (mme_ue_s1ap_id_ue_context_htbl, (const hash_key_t)mme_ue_s1ap_id, (void *)(uintptr_t)enb_s1ap_id_key);
     if (HASH_TABLE_OK != h_rc) {
        OAILOG_ERROR (LOG_MME_APP,
    ...
    if (INVALID_IMSI64 != ue_context_p->emm_context._imsi64) {
      h_rc = hashtable_ts_remove (imsi_ue_context_htbl, (const hash_key_t)ue_context_p->emm_context._imsi64, (void **)&id);
      h_rc = hashtable_ts_insert (imsi_ue_context_htbl, (const hash_key_t)ue_context_p->emm_context._imsi64, (void *)(uintptr_t)mme_ue_s1ap_id);
    }
    h_rc = hashtable_ts_remove (tun11_ue_context_htbl, (const hash_key_t)ue_context_p->mme_teid_s11, (void **)&id);
    h_rc = hashtable_ts_insert (tun11_ue_context_htbl, (const hash_key_t)ue_context_p->mme_teid_s11, (void *)(uintptr_t)mme_ue_s1ap_id);

    h_rc = obj_hashtable_ts_remove (guti_ue_context_htbl, (const void *const)&ue_context_p->emm_context._guti,
        sizeof (ue_context_p->emm_context._guti), (void **)&id);
    if (INVALID_MME_UE_S1AP_ID != mme_ue_s1ap_id) {
      h_rc = obj_hashtable_ts_insert (guti_ue_context_htbl, (const void *const)&ue_context_p->emm_context._guti,
          sizeof (ue_context_p->emm_context._guti), (void *)(uintptr_t)mme_ue_s1ap_id);
    }
  }
  ...
  if ((ue_context_p->emm_context._imsi64 != imsi)
    ...
    h_rc = hashtable_ts_remove (imsi_ue_context_htbl, (const hash_key_t)ue_context_p->emm_context._imsi64, (void **)&id);
    if (INVALID_MME_UE_S1AP_ID != mme_ue_s1ap_id) {
      h_rc = hashtable_ts_insert (imsi_ue_context_htbl, (const hash_key_t)imsi, (void *)(uintptr_t)mme_ue_s1ap_id);
    } else {
  ...
  if ((ue_context_p->mme_teid_s11 != mme_teid_s11)
    ...
    h_rc = hashtable_ts_remove (tun11_ue_context_htbl, (const hash_key_t)ue_context_p->mme_teid_s11, (void **)&id);
    if (INVALID_MME_UE_S1AP_ID != mme_ue_s1ap_id) {
      h_rc = hashtable_ts_insert (tun11_ue_context_htbl, (const hash_key_t)mme_teid_s11, (void *)(uintptr_t)mme_ue_s1ap_id);
    } else {
      h_rc = HASH_TABLE_KEY_NOT_EXISTS;
    }
  ...
  if (guti_p) {
    if (guti_cmp(guti_p, &(ue_context_p->emm_context._guti)) ||
        (ue_context_p->mme_ue_s1ap_id != mme_ue_s1ap_id)) {

      // may check guti_p with a kind of instanceof()?
      h_rc = obj_hashtable_ts_remove (guti_ue_context_htbl, &ue_context_p->emm_context._guti, sizeof (*guti_p), (void **)&id);
      if (INVALID_MME_UE_S1AP_ID != mme_ue_s1ap_id) {
        h_rc = obj_hashtable_ts_insert (guti_ue_context_htbl, (const void *const)guti_p, sizeof (*guti_p), (void *)(uintptr_t)mme_ue_s1ap_id);
      } else {
        h_rc = HASH_TABLE_KEY_NOT_EXISTS;
      }
...
//------------------------------------------------------------------------------
void MmeUeContextManager::DumpCollKeys() {
  bstring tmp = bfromcstr(" ");
  btrunc(tmp, 0);

  hashtable_ts_dump_content (imsi_ue_context_htbl, tmp);
  OAILOG_TRACE (LOG_MME_APP,"imsi_ue_context_htbl %s\n", bdata(tmp));

  btrunc(tmp, 0);
  hashtable_ts_dump_content (tun11_ue_context_htbl, tmp);
  OAILOG_TRACE (LOG_MME_APP,"tun11_ue_context_htbl %s\n", bdata(tmp));

  btrunc(tmp, 0);
  hashtable_ts_dump_content (mme_ue_s1ap_id_ue_context_htbl, tmp);
  OAILOG_TRACE (LOG_MME_APP,"mme_ue_s1ap_id_ue_context_htbl %s\n", bdata(tmp));

  btrunc(tmp, 0);
  hashtable_ts_dump_content (enb_ue_s1ap_id_ue_context_htbl, tmp);
  OAILOG_TRACE (LOG_MME_APP,"enb_ue_s1ap_id_ue_context_htbl %s\n", bdata(tmp));

  btrunc(tmp, 0);
  obj_hashtable_ts_dump_content (guti_ue_context_htbl, tmp);
  OAILOG_TRACE (LOG_MME_APP,"guti_ue_context_htbl %s", bdata(tmp));
}
//------------------------------------------------------------------------------

int MmeUeContextManager::InsertUeContext(const ue_mm_context_t * const ue_context_p) {
  hashtable_rc_t                          h_rc = HASH_TABLE_OK;
  ...
  h_rc = hashtable_ts_is_key_exists (enb_ue_s1ap_id_ue_context_htbl, (const hash_key_t)ue_context_p->enb_s1ap_id_key);
  if (HASH_TABLE_OK == h_rc) {
  ...
  h_rc = hashtable_ts_insert (enb_ue_s1ap_id_ue_context_htbl,
  ...
  if ( INVALID_MME_UE_S1AP_ID != ue_context_p->mme_ue_s1ap_id) {
    // filled MME UE S1AP ID
    h_rc = hashtable_ts_is_key_exists (mme_ue_s1ap_id_ue_context_htbl, (const hash_key_t)ue_context_p->mme_ue_s1ap_id);
    ...
    h_rc = hashtable_ts_insert (mme_ue_s1ap_id_ue_context_htbl,
    ...
    if (ue_context_p->emm_context._imsi64) {
      h_rc = hashtable_ts_insert (imsi_ue_context_htbl,
    ...
    if (ue_context_p->mme_teid_s11) {
      h_rc = hashtable_ts_insert (tun11_ue_context_htbl,
    ...
    // filled guti
    if ( if_guti_valid(&ue_context_p->emm_context._guti)) {
      h_rc = obj_hashtable_ts_insert (guti_ue_context_htbl,
     ...
     * Updating statistics
     */
  __sync_fetch_and_add (&nb_ue_managed, 1);
  __sync_fetch_and_add (&nb_ue_since_last_stat, 1);

  OAILOG_FUNC_RETURN (LOG_MME_APP, RETURNok);
}

//------------------------------------------------------------------------------
void MmeUeContextManager::NotifyUeContextReleased(ue_mm_context_t *ue_context_p) {
   ...
   * Updating statistics
   */
  __sync_fetch_and_sub (&nb_ue_managed, 1);
  __sync_fetch_and_sub (&nb_ue_since_last_stat, 1);
...
//------------------------------------------------------------------------------
void MmeUeContextManager::RemoveUeContext(ue_mm_context_t * const ue_context_p) {
  ...
  // remove IMSI
  if (ue_context_p->emm_context._imsi64) {
    hash_rc = hashtable_ts_remove (imsi_ue_context_htbl, (const hash_key_t)ue_context_p->emm_context._imsi64, (void **)&id);
  ...
  // remove NAS UE ID (MME UE S1AP ID)
  if (INVALID_MME_UE_S1AP_ID != ue_context_p->mme_ue_s1ap_id) {
    hash_rc = hashtable_ts_remove (mme_ue_s1ap_id_ue_context_htbl, (const hash_key_t)ue_context_p->mme_ue_s1ap_id, (void **)&id);
    if (HASH_TABLE_OK != hash_rc)
  ...
  // remove S11 tun id
  if (ue_context_p->mme_teid_s11) {
    hash_rc = hashtable_ts_remove (tun11_ue_context_htbl, (const hash_key_t)ue_context_p->mme_teid_s11, (void **)&id);
    if (HASH_TABLE_OK != hash_rc)
  ...
  // remove guti
  if (if_guti_valid(&ue_context_p->emm_context._guti)) {
    hash_rc = obj_hashtable_ts_remove (guti_ue_context_htbl, (const void *const)&ue_context_p->emm_context._guti, sizeof (ue_context_p->emm_context._guti), (void **)&id);
    if (HASH_TABLE_OK != hash_rc)
  ...
  hash_rc = hashtable_ts_remove (enb_ue_s1ap_id_ue_context_htbl, (const hash_key_t)ue_context_p->enb_s1ap_id_key, (void **)&ue_context_p);
  if (HASH_TABLE_OK != hash_rc)
...
//------------------------------------------------------------------------------
void mme_app_dump_pdn_context (const ue_mm_context_t *const ue_mm_context,
...
//------------------------------------------------------------------------------
{
  ue_mm_context_t           *const ue_mm_context = (ue_mm_context_t *)ue_mm_context_pP;
  uint8_t                                 j = 0;
  if (ue_mm_context) {
...
void MmeUeContextManager::DumpUeContexts() {
  hashtable_ts_apply_callback_on_elements (
              this->enb_ue_s1ap_id_ue_context_htbl,
              mme_app_dump_ue_context,
              NULL, NULL);
}
...
//------------------------------------------------------------------------------
{
  ...
  ue_context_p = mme_app_desc.GetUeContextByMmeUeS1apId(s1ap_ue_context_release_req->mme_ue_s1ap_id);
  if (!ue_context_p) {
    ue_context_p = mme_app_desc.GetUeContextByEnbUeS1apId(s1ap_ue_context_release_req->enb_ue_s1ap_id);
...
//------------------------------------------------------------------------------
{
  ...
  ue_context_p = mme_app_desc.GetUeContextByMmeUeS1apId(s1ap_ue_context_release_complete->mme_ue_s1ap_id);
  ...
  mme_app_desc.NotifyUeContextReleased(ue_context_p);
