...
static void mme_app_send_delete_session_request (ue_mm_context_t * const ue_context_p, const ebi_t ebi, const pdn_cid_t cid);
...
static void mme_app_send_delete_session_request (ue_mm_context_t * const ue_context_p, const ebi_t ebi, const pdn_cid_t cid)
{
  ...
  S11_DELETE_SESSION_REQUEST (message_p).sender_fteid_for_cp.teid = reinterpret_cast<uintptr_t>(ue_context_p); // may lead to bug!
  ...
  const itti_nas_detach_req_t * const detach_req_p)
{
  ...
  ue_context = mme_app_desc.GetUeContextByMmeUeS1apId(detach_req_p->ue_id);
  if (ue_context == NULL) {
