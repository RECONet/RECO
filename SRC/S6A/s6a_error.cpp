...
s6a_parse_experimental_result(
  ...
  CHECK_FCT(fd_msg_browse(avp, MSG_BRW_FIRST_CHILD, \
  ...
  while (child_avp) {
    ...
    case AVP_CODE_VENDOR_ID:
      DevCheck(hdr->avp_value->u32 == 10415, \
               hdr->avp_value->u32, AVP_CODE_VENDOR_ID, 10415);
      break;
     ...
     */
    CHECK_FCT(fd_msg_browse(child_avp, MSG_BRW_NEXT, \
              (msg_or_avp**)&child_avp, NULL));
  }
