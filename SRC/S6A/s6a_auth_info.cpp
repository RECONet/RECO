...
#include "s6a_config.h"
  ...
  int
s6a_auth_info_base::s6a_parse_rand (
  struct avp_hdr *hdr,
  uint8_t * rand_p)
{
  int                                     ret = 0;
  ...
  int
s6a_auth_info_base::s6a_parse_xres (
  struct avp_hdr *hdr,
  res_t * xres)
{
  int                                     ret = 0;
  ...
  int
s6a_auth_info_base::s6a_parse_autn (
  struct avp_hdr *hdr,
  uint8_t * autn)
{
  int                                     ret = 0;
  ...
  int
s6a_auth_info_base::s6a_parse_kasme (
  struct avp_hdr *hdr,
  uint8_t * kasme)
{
  int                                     ret = 0;
...
inline int
s6a_auth_info_base::s6a_parse_e_utran_vector (
  ...
  eutran_vector_t * vector)
{
  ...
  CHECK_FCT (fd_msg_browse (avp_vector, MSG_BRW_FIRST_CHILD, (msg_or_avp**)&avp, NULL));
...
inline int
s6a_auth_info_base::s6a_parse_authentication_info_avp (
  ...
  authentication_info_t * authentication_info)
{
  ...
  CHECK_FCT (fd_msg_browse (avp_auth_info, MSG_BRW_FIRST_CHILD, (msg_or_avp**)&avp, NULL));
  ...
  enum disp_action *act)
{
  ...
  if (avp) {
    ...
    s6a_auth_info_ans_p->result.choice.base = (s6a_base_result_t)hdr->avp_value->u32;
  ...
  if (skip_auth_res == 0) {
    ...
    if (avp) {
      s6a_auth_info_base ai;
      CHECK_FCT (ai.s6a_parse_authentication_info_avp (avp, &s6a_auth_info_ans_p->auth_info));
    } else {
...
int
s6a_auth_info_base::s6a_generate_authentication_info_req (
