...
s6a_cxx::s6a_fd_init_dict_objs(
  ...
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_VENDOR, VENDOR_BY_ID, (void *)&vendor_3gpp, \
               &s6a_fd_cnf.dataobj_s6a_vendor, ENOENT));
  /*
   * Pre-loading application object
   */
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_APPLICATION, APPLICATION_BY_ID, (void *)&app_s6a, \
               &s6a_fd_cnf.dataobj_s6a_app, ENOENT));
  /*
   * Pre-loading commands objects
   */
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_COMMAND, CMD_BY_NAME, \
               "Authentication-Information-Request", \
               &s6a_fd_cnf.dataobj_s6a_air, ENOENT));
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_COMMAND, CMD_BY_NAME, "Authentication-Information-Answer", \
               &s6a_fd_cnf.dataobj_s6a_aia, ENOENT));
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_COMMAND, CMD_BY_NAME, "Update-Location-Request", \
               &s6a_fd_cnf.dataobj_s6a_ulr, ENOENT));
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_COMMAND, CMD_BY_NAME, "Update-Location-Answer", \
               &s6a_fd_cnf.dataobj_s6a_ula, ENOENT));
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_COMMAND, CMD_BY_NAME, "Purge-UE-Request", \
               &s6a_fd_cnf.dataobj_s6a_pur, ENOENT));
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_COMMAND, CMD_BY_NAME, "Purge-UE-Answer", \
               &s6a_fd_cnf.dataobj_s6a_pua, ENOENT));
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_COMMAND, CMD_BY_NAME, "Cancel-Location-Request", \
               &s6a_fd_cnf.dataobj_s6a_clr, ENOENT));
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_COMMAND, CMD_BY_NAME, "Cancel-Location-Answer", \
               &s6a_fd_cnf.dataobj_s6a_cla, ENOENT));
  /*
   * Pre-loading base avps
   */
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_AVP, AVP_BY_NAME, "Destination-Host", \
               &s6a_fd_cnf.dataobj_s6a_destination_host, ENOENT));
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_AVP, AVP_BY_NAME, "Destination-Realm", \
               &s6a_fd_cnf.dataobj_s6a_destination_realm, ENOENT));
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_AVP, AVP_BY_NAME, "User-Name", \
               &s6a_fd_cnf.dataobj_s6a_user_name, ENOENT));
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_AVP, AVP_BY_NAME, "Session-Id", \
               &s6a_fd_cnf.dataobj_s6a_session_id, ENOENT));
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_AVP, AVP_BY_NAME, "Auth-Session-State", \
               &s6a_fd_cnf.dataobj_s6a_auth_session_state, ENOENT));
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_AVP, AVP_BY_NAME, "Result-Code", \
               &s6a_fd_cnf.dataobj_s6a_result_code, ENOENT));
  CHECK_FD_FCT(fd_dict_search(fd_g_config->cnf_dict, \
               DICT_AVP, AVP_BY_NAME, "Experimental-Result", \
   ...
   */
  s6a_ula_cb_cxx s6a_ula_p = s6a_up_loc::s6a_ula_cb;
  CHECK_FD_FCT(fd_disp_register(s6a_ula_p, DISP_HOW_CC, \
               &when, NULL, &s6a_fd_cnf.ula_hdl));
   ...
   */
  // s6a_auth_info ai;
  // ai.s6a_aia_cb_cxx = &s6a_auth_info::s6a_aia_cb;
  s6a_aia_cb_cxx s6a_aia_p = s6a_auth_info::s6a_aia_cb;
  CHECK_FD_FCT(fd_disp_register(s6a_aia_p, DISP_HOW_CC, \
               &when, NULL, &s6a_fd_cnf.aia_hdl));
  // CHECK_FD_FCT (fd_disp_register (s6a_aia_cb_cxx, \
     DISP_HOW_CC, &when, NULL, &s6a_fd_cnf.aia_hdl));
