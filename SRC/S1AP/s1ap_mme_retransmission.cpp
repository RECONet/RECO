...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_timer_insert (
  const mme_ue_s1ap_id_t mme_ue_s1ap_id,
  const long timer_id)
{
  struct s1ap_timer_map_s                *new_map = NULL;

  new_map = static_cast<s1ap_timer_map_s*>(malloc (sizeof (struct s1ap_timer_map_s)));
  new_map->timer_id = timer_id;
  new_map->mme_ue_s1ap_id = mme_ue_s1ap_id;

  if (RB_INSERT (s1ap_timer_map, &s1ap_timer_tree, new_map) != NULL) {
    OAILOG_WARNING (LOG_S1AP, "Timer with id 0x%lx already exists\n", timer_id);
    free_wrapper ((void**)&new_map);
    return -1;
  }
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_handle_timer_expiry (
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_timer_remove_ue (
  const mme_ue_s1ap_id_t mme_ue_s1ap_id)
{
  struct s1ap_timer_map_s                *find = NULL;
