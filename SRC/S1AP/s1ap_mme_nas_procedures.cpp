...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_handle_initial_ue_message (
  ...
  struct s1ap_message_s *message)
{
  ...
  if (ue_ref == NULL) {
    tai_t                                   tai = {0};
    gummei_t                                gummei = {plmn : {0}, mme_gid : 0, mme_code : 0}; // initialized after
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_handle_uplink_nas_transport (
  sctp_assoc_id_t assoc_id,
  sctp_stream_id_t stream,
  struct s1ap_message_s *message)
{
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_handle_nas_non_delivery (
  sctp_assoc_id_t assoc_id,
  sctp_stream_id_t stream,
  struct s1ap_message_s *message)
{
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_generate_downlink_nas_transport (
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_generate_s1ap_e_rab_setup_req (itti_s1ap_e_rab_setup_req_t * const e_rab_setup_req)
{
  OAILOG_FUNC_IN (LOG_S1AP);
  ue_description_t                       *ue_ref = NULL;
  uint8_t                                *buffer_p = NULL;
...
//    }
    ...
    for  (int i= 0; i < e_rab_setup_req->e_rab_to_be_setup_list.no_of_items; i++) {
      ...
      s1ap_E_RABToBeSetupItemBearerSUReq[i].transportLayerAddress.buf = static_cast<uint8_t*> (calloc (blength(e_rab_setup_req->e_rab_to_be_setup_list.item[i].transport_layer_address), sizeof (uint8_t)));
...
//------------------------------------------------------------------------------
void s1ap_mme_cxx::s1ap_handle_conn_est_cnf (
  const itti_mme_app_connection_establishment_cnf_t * const conn_est_cnf_pP)
{
  ...
  for (int item = 0; item < conn_est_cnf_pP->no_of_e_rabs; item++) {
    ...
    nas_pdu.buf  = static_cast<uint8_t*>(malloc(blength(conn_est_cnf_pP->nas_pdu[item])));
    ...
    e_RABToBeSetup[item].transportLayerAddress.buf = static_cast<uint8_t*>(calloc (blength(conn_est_cnf_pP->transport_layer_address[item]), sizeof (uint8_t)));
  ...
  if (conn_est_cnf_pP->kenb) {
    initialContextSetupRequest_p->securityKey.buf = static_cast<uint8_t*>(calloc (AUTH_KENB_SIZE, sizeof(uint8_t)));
