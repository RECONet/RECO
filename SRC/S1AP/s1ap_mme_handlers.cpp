...
s1ap_mme_cxx::s1ap_message_decoded_callback_cxx           messages_callback[][3] = {
  {0, 0, 0},                    /* HandoverPreparation */
  {0, 0, 0},                    /* HandoverResourceAllocation */
  {0, 0, 0},                    /* HandoverNotification */
  {&s1ap_mme_cxx::s1ap_mme_handle_path_switch_request, 0, 0},  /* PathSwitchRequest */
  {0, 0, 0},                    /* HandoverCancel */
  {0, &s1ap_mme_cxx::s1ap_mme_handle_erab_setup_response,
      &s1ap_mme_cxx::s1ap_mme_handle_erab_setup_failure},                    /* E_RABSetup */
  {0, 0, 0},                    /* E_RABModify */
  {0, 0, 0},                    /* E_RABRelease */
  {0, 0, 0},                    /* E_RABReleaseIndication */
  {
   0, &s1ap_mme_cxx::s1ap_mme_handle_initial_context_setup_response,
   &s1ap_mme_cxx::s1ap_mme_handle_initial_context_setup_failure},      /* InitialContextSetup */
  {0, 0, 0},                    /* Paging */
  {0, 0, 0},                    /* downlinkNASTransport */
  {&s1ap_mme_cxx::s1ap_mme_handle_initial_ue_message, 0, 0},   /* initialUEMessage */
  {&s1ap_mme_cxx::s1ap_mme_handle_uplink_nas_transport, 0, 0}, /* uplinkNASTransport */
  {0, 0, 0},                    /* Reset */
  {0, 0, 0},                    /* ErrorIndication */
  {&s1ap_mme_cxx::s1ap_mme_handle_nas_non_delivery, 0, 0},     /* NASNonDeliveryIndication */
  {&s1ap_mme_cxx::s1ap_mme_handle_s1_setup_request, 0, 0},     /* S1Setup */
  {&s1ap_mme_cxx::s1ap_mme_handle_ue_context_release_request, 0, 0},   /* UEContextReleaseRequest */
  {0, 0, 0},                    /* DownlinkS1cdma2000tunneling */
  {0, 0, 0},                    /* UplinkS1cdma2000tunneling */
  {0, 0, 0},                    /* UEContextModification */
  {&s1ap_mme_cxx::s1ap_mme_handle_ue_cap_indication, 0, 0},    /* UECapabilityInfoIndication */
  {
   &s1ap_mme_cxx::s1ap_mme_handle_ue_context_release_request,
   &s1ap_mme_cxx::s1ap_mme_handle_ue_context_release_complete, 0},     /* UEContextRelease */
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_handle_message (
    ...
    struct s1ap_message_s *message)
{
  ...
  return (this->*messages_callback[message->procedureCode][message->direction - 1]) (assoc_id, stream, message);
}

//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_set_cause (
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_generate_s1_setup_failure (
    const sctp_assoc_id_t assoc_id,
    const S1ap_Cause_PR cause_type,
    const long cause_value,
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_handle_s1_setup_request (
    const sctp_assoc_id_t assoc_id,
    const sctp_stream_id_t stream,
    struct s1ap_message_s *message)
{
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_generate_s1_setup_response (
  enb_description_t * enb_association)
{
  ...
  for (i = 0; i < mme_config.served_tai.nb_tai; i++) {
    ...
    if (false == plmn_added) {
       ...
       */
      plmn = static_cast<S1ap_PLMNidentity_t*>(calloc (1, sizeof (*plmn)));
  ...
  for (i = 0; i < mme_config.gummei.nb; i++) {
     ...
     * FIXME: free object from list once encoded
     */
    mme_gid = static_cast<S1ap_MME_Group_ID_t*> (calloc (1, sizeof (*mme_gid)));
     ...
     * FIXME: free object from list once encoded
     */
    mmec = static_cast<S1ap_MME_Code_t*> (calloc (1, sizeof (*mmec)));
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_handle_ue_cap_indication (
    const sctp_assoc_id_t assoc_id,
    const sctp_stream_id_t stream,
    struct s1ap_message_s *message)
{
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_handle_initial_context_setup_response (
    const sctp_assoc_id_t assoc_id,
    const sctp_stream_id_t stream,
    struct s1ap_message_s *message)
{
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_handle_ue_context_release_request (
    const sctp_assoc_id_t assoc_id,
    const sctp_stream_id_t stream,
    struct s1ap_message_s *message)
{
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_generate_ue_context_release_command (ue_description_t * ue_ref_p, S1ap_Cause_t cause)
{
  uint8_t                                *buffer = NULL;
  uint32_t                                length = 0;
  s1ap_message                            message = {0};
...
//------------------------------------------------------------------------------
void s1ap_mme_cxx::s1ap_handle_delete_session_rsp (
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_handle_ue_context_release_command (
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_handle_ue_context_release_complete (
    const sctp_assoc_id_t assoc_id,
    const sctp_stream_id_t stream,
    struct s1ap_message_s *message)
{
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_handle_initial_context_setup_failure (
    const sctp_assoc_id_t assoc_id,
    const sctp_stream_id_t stream,
    struct s1ap_message_s *message)
{
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_handle_path_switch_request (
    const sctp_assoc_id_t assoc_id,
    const sctp_stream_id_t stream,
    struct s1ap_message_s *message)
{
...
//------------------------------------------------------------------------------
bool s1ap_mme_cxx::s1ap_send_enb_deregistered_ind (
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_handle_sctp_deconnection (
    const sctp_assoc_id_t assoc_id)
{
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_handle_new_association (
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_handle_erab_setup_response (
    const sctp_assoc_id_t assoc_id,
    const sctp_stream_id_t stream,
    struct s1ap_message_s *message)
{
...
//------------------------------------------------------------------------------
int s1ap_mme_cxx::s1ap_mme_handle_erab_setup_failure (const sctp_assoc_id_t assoc_id,
