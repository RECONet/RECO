...
//------------------------------------------------------------------------------
void emm_ctx_mark_common_procedure_running(emm_context_t * const ctxt, const int proc_id)
{
  __sync_fetch_and_or(&ctxt->common_proc_mask, proc_id);
}

void emm_ctx_unmark_common_procedure_running(emm_context_t * const ctxt, const int proc_id)
{
  __sync_fetch_and_and(&ctxt->common_proc_mask, ~proc_id);
}

bool emm_ctx_is_common_procedure_running(emm_context_t * const ctxt, const int proc_id)
{
  if ((ctxt) && (ctxt->common_proc_mask & proc_id)) return true;
  return false;
}

void emm_ctx_mark_specific_procedure_running(emm_context_t * const ctxt, const int proc_id)
{
  __sync_fetch_and_or(&ctxt->specific_proc_mask, proc_id);
}

void emm_ctx_unmark_specific_procedure_running(emm_context_t * const ctxt, const int proc_id)
{
  __sync_fetch_and_and(&ctxt->specific_proc_mask, ~proc_id);
}

bool emm_ctx_is_specific_procedure_running(emm_context_t * const ctxt, const int proc_id)
{
...
//------------------------------------------------------------------------------
void emm_ctx_set_attribute_present(emm_context_t * const ctxt, const int attribute_bit_pos)
{
  ctxt->member_present_mask |= attribute_bit_pos;
}

void emm_ctx_clear_attribute_present(emm_context_t * const ctxt, const int attribute_bit_pos)
{
  ctxt->member_present_mask &= ~attribute_bit_pos;
  ctxt->member_valid_mask &= ~attribute_bit_pos;
}

void emm_ctx_set_attribute_valid(emm_context_t * const ctxt, const int attribute_bit_pos)
{
  ctxt->member_present_mask |= attribute_bit_pos;
  ctxt->member_valid_mask   |= attribute_bit_pos;
}

void emm_ctx_clear_attribute_valid(emm_context_t * const ctxt, const int attribute_bit_pos)
{
  ctxt->member_valid_mask &= ~attribute_bit_pos;
}
...
void emm_ctx_set_guti(emm_context_t * const ctxt, guti_t *guti)
{
  ...
  OAILOG_DEBUG (LOG_NAS_EMM, "ue_id=" MME_UE_S1AP_ID_FMT " set GUTI " GUTI_FMT " (present)\n", (PARENT_STRUCT(ctxt, ue_mm_context_t, emm_context))->mme_ue_s1ap_id, GUTI_ARG(&ctxt->_guti));
}
...
void emm_ctx_set_old_guti(emm_context_t * const ctxt, guti_t *guti)
{
  ...
  OAILOG_DEBUG (LOG_NAS_EMM, "ue_id=" MME_UE_S1AP_ID_FMT " set old GUTI " GUTI_FMT " (present)\n", (PARENT_STRUCT(ctxt, ue_mm_context_t, emm_context))->mme_ue_s1ap_id, GUTI_ARG(&ctxt->_old_guti));
}
...
/* Set IMSI */
void emm_ctx_set_imsi(emm_context_t * const ctxt, imsi_t *imsi, const imsi64_t imsi64)
{
  ctxt->_imsi   = *imsi;
  ctxt->_imsi64 = imsi64;
  ...
  OAILOG_DEBUG (LOG_NAS_EMM, "ue_id=" MME_UE_S1AP_ID_FMT " set IMSI %s (valid)\n", (PARENT_STRUCT(ctxt, ue_mm_context_t, emm_context))->mme_ue_s1ap_id, imsi_str);
#endif
}

/* Set IMSI, mark it as valid */
void emm_ctx_set_valid_imsi(emm_context_t * const ctxt, imsi_t *imsi, const imsi64_t imsi64)
{
  ctxt->_imsi   = *imsi;
  ctxt->_imsi64 = imsi64;
  ...
  OAILOG_DEBUG (LOG_NAS_EMM, "ue_id=" MME_UE_S1AP_ID_FMT " set IMSI %s (valid)\n", (PARENT_STRUCT(ctxt, ue_mm_context_t, emm_context))->mme_ue_s1ap_id, imsi_str);
#endif
  mme_api_notify_imsi((PARENT_STRUCT(ctxt, ue_mm_context_t, emm_context))->mme_ue_s1ap_id, imsi64);
  hashtable_ts_insert (_emm_data.ctx_coll_imsi, imsi64, (void*)((uintptr_t )(PARENT_STRUCT(ctxt, ue_mm_context_t, emm_context))->mme_ue_s1ap_id));
}
...
void emm_ctx_set_imei(emm_context_t * const ctxt, imei_t *imei)
{
  ...
  OAILOG_DEBUG (LOG_NAS_EMM, "ue_id=" MME_UE_S1AP_ID_FMT " set IMEI %s (present)\n", (PARENT_STRUCT(ctxt, ue_mm_context_t, emm_context))->mme_ue_s1ap_id, imei_str);
#endif
}
...
//------------------------------------------------------------------------------
/* Clear IMEI_SV */
void emm_ctx_clear_imeisv(emm_context_t * const ctxt)
{
...
void emm_ctx_clear_lvr_tai(emm_context_t * const ctxt)
{
...
void emm_ctx_set_lvr_tai(emm_context_t * const ctxt, tai_t *lvr_tai)
{
  ...
  //    "ue_id="MME_UE_S1AP_ID_FMT" set last visited registered TAI "TAI_FMT" (present)\n", (PARENT_STRUCT(ctxt, ue_mm_context_t, emm_context))->mme_ue_s1ap_id, TAI_ARG(&ctxt->_lvr_tai));
...
//------------------------------------------------------------------------------
void emm_ctx_set_security_vector_index(emm_context_t * const ctxt, int vector_index)
{
  ctxt->_security.vector_index = vector_index;
  OAILOG_TRACE (LOG_NAS_EMM, "ue_id=" MME_UE_S1AP_ID_FMT " set security context vector index %d\n", (PARENT_STRUCT(ctxt, ue_mm_context_t, emm_context))->mme_ue_s1ap_id, vector_index);
}
...
//------------------------------------------------------------------------------
void emm_ctx_set_non_current_security_vector_index(emm_context_t * const ctxt, int vector_index)
{
...
void emm_ctx_set_ue_nw_cap(emm_context_t * const ctxt, const ue_network_capability_t * const ue_nw_cap_ie)
{
...
void emm_ctx_set_valid_ue_nw_cap(emm_context_t * const ctxt, const ue_network_capability_t * const ue_nw_cap_ie)
{
  ctxt->_ue_network_capability = *ue_nw_cap_ie;
...
void emm_ctx_set_current_drx_parameter(emm_context_t * const ctxt, drx_parameter_t *drx)
{
...
void emm_ctx_set_valid_current_drx_parameter(emm_context_t * const ctxt, drx_parameter_t *drx)
{
  ctxt->_current_drx_parameter = *drx;
...
/* Set current DRX parameter */
void emm_ctx_set_pending_current_drx_parameter(emm_context_t * const ctxt, drx_parameter_t *drx)
{
...
/* Set current DRX parameter, mark it as valid */
void emm_ctx_set_valid_pending_current_drx_parameter(emm_context_t * const ctxt, drx_parameter_t *drx)
{
  ctxt->_pending_drx_parameter = *drx;
...
/* Set current DRX parameter */
void emm_ctx_set_eps_bearer_context_status(emm_context_t * const ctxt, eps_bearer_context_status_t *status)
{
...
/* Set current DRX parameter, mark it as valid */
void emm_ctx_set_valid_eps_bearer_context_status(emm_context_t * const ctxt, eps_bearer_context_status_t *status)
{
  ctxt->_eps_bearer_context_status = *status;
  ...
  struct emm_context_s *elm)
{
  ...
  hashtable_ts_remove (emm_data->ctx_coll_ue_id, (const hash_key_t)((PARENT_STRUCT(elm, ue_mm_context_t, emm_context))->mme_ue_s1ap_id), (void **)&emm_context_p);
  ...
  struct emm_context_s *elm)
{
  ...
  if ( IS_EMM_CTXT_PRESENT_GUTI(elm)) {
    h_rc = obj_hashtable_ts_insert (emm_data->ctx_coll_guti, (const void *const)(&elm->_guti), sizeof (elm->_guti), (void*)((uintptr_t )(PARENT_STRUCT(elm, ue_mm_context_t, emm_context))->mme_ue_s1ap_id));

    if (HASH_TABLE_OK == h_rc) {
      OAILOG_DEBUG (LOG_NAS_EMM, "EMM-CTX - Add in context UE id " MME_UE_S1AP_ID_FMT " with GUTI "GUTI_FMT"\n", (PARENT_STRUCT(elm, ue_mm_context_t, emm_context))->mme_ue_s1ap_id, GUTI_ARG(&elm->_guti));
    } else {
      OAILOG_ERROR (LOG_NAS_EMM, "EMM-CTX - Add in context UE id " MME_UE_S1AP_ID_FMT " with GUTI "GUTI_FMT" Failed %s\n", (PARENT_STRUCT(elm, ue_mm_context_t, emm_context))->mme_ue_s1ap_id, GUTI_ARG(&elm->_guti), hashtable_rc_code2string (h_rc));
      return RETURNerror;
    }
  }
  return RETURNok;
  ...
  struct emm_context_s *elm)
{
  ...
  if ( IS_EMM_CTXT_PRESENT_OLD_GUTI(elm)) {
    h_rc = obj_hashtable_ts_insert (emm_data->ctx_coll_guti, (const void *const)(&elm->_old_guti), sizeof (elm->_old_guti), (void*)((uintptr_t )(PARENT_STRUCT(elm, ue_mm_context_t, emm_context))->mme_ue_s1ap_id));

    if (HASH_TABLE_OK == h_rc) {
      OAILOG_DEBUG (LOG_NAS_EMM, "EMM-CTX - Add in context UE id " MME_UE_S1AP_ID_FMT " with old GUTI "GUTI_FMT"\n", (PARENT_STRUCT(elm, ue_mm_context_t, emm_context))->mme_ue_s1ap_id, GUTI_ARG(&elm->_old_guti));
    } else {
      OAILOG_ERROR (LOG_NAS_EMM, "EMM-CTX - Add in context UE id " MME_UE_S1AP_ID_FMT " with old GUTI "GUTI_FMT" Failed %s\n", (PARENT_STRUCT(elm, ue_mm_context_t, emm_context))->mme_ue_s1ap_id, GUTI_ARG(&elm->_old_guti), hashtable_rc_code2string (h_rc));
      return RETURNerror;
    }
  }
  return RETURNok;
  ...
  struct emm_context_s *elm)
{
  ...
  if ( IS_EMM_CTXT_PRESENT_IMSI(elm)) {
    h_rc = hashtable_ts_insert (emm_data->ctx_coll_imsi, elm->_imsi64, (void*)((uintptr_t )(PARENT_STRUCT(elm, ue_mm_context_t, emm_context))->mme_ue_s1ap_id));

    if (HASH_TABLE_OK == h_rc) {
      OAILOG_DEBUG (LOG_NAS_EMM, "EMM-CTX - Add in context UE id " MME_UE_S1AP_ID_FMT " with IMSI "IMSI_64_FMT"\n", (PARENT_STRUCT(elm, ue_mm_context_t, emm_context))->mme_ue_s1ap_id, elm->_imsi64);
    } else {
      OAILOG_ERROR (LOG_NAS_EMM, "EMM-CTX - Add in context UE id " MME_UE_S1AP_ID_FMT " with IMSI "IMSI_64_FMT" Failed %s\n",
          (PARENT_STRUCT(elm, ue_mm_context_t, emm_context))->mme_ue_s1ap_id, elm->_imsi64, hashtable_rc_code2string (h_rc));
      return RETURNerror;
    }
  }
  return RETURNok;
...
void emm_init_context(struct emm_context_s * const emm_ctx, const bool init_esm_ctxt)
{
    ...
    emm_ctx->T3450.sec       = nas_config.t3450_sec;
    emm_ctx->T3460.id        = NAS_TIMER_INACTIVE_ID;
    emm_ctx->T3460.sec       = nas_config.t3460_sec;
    emm_ctx->T3470.id        = NAS_TIMER_INACTIVE_ID;
    emm_ctx->T3470.sec       = nas_config.t3470_sec;
    ue_mm_context_t * ue_mm_context = PARENT_STRUCT(emm_ctx, ue_mm_context_t, emm_context);
  ...
  if (emm_ctx ) {
    ...
    if (emm_ctx->T3450.id != NAS_TIMER_INACTIVE_ID) {
      ...
      if (NAS_TIMER_INACTIVE_ID == emm_ctx->T3450.id) {
        MSC_LOG_EVENT (MSC_NAS_EMM_MME, "0 T3450 stopped UE " MME_UE_S1AP_ID_FMT " ", (PARENT_STRUCT(emm_ctx, ue_mm_context_t, emm_context))->mme_ue_s1ap_id);
        OAILOG_DEBUG (LOG_NAS_EMM, "T3450 stopped UE " MME_UE_S1AP_ID_FMT " ", (PARENT_STRUCT(emm_ctx, ue_mm_context_t, emm_context))->mme_ue_s1ap_id);
      } else {
        OAILOG_ERROR (LOG_NAS_EMM, "Could not stop T3450 stopped UE " MME_UE_S1AP_ID_FMT " ", (PARENT_STRUCT(emm_ctx, ue_mm_context_t, emm_context))->mme_ue_s1ap_id);
      }
    }
    ...
    if (emm_ctx->T3460.id != NAS_TIMER_INACTIVE_ID) {
      ...
      if (NAS_TIMER_INACTIVE_ID == emm_ctx->T3460.id) {
        MSC_LOG_EVENT (MSC_NAS_EMM_MME, "0 T3460 stopped UE " MME_UE_S1AP_ID_FMT " ", (PARENT_STRUCT(emm_ctx, ue_mm_context_t, emm_context))->mme_ue_s1ap_id);
        OAILOG_DEBUG (LOG_NAS_EMM, "T3460 stopped UE " MME_UE_S1AP_ID_FMT " ", (PARENT_STRUCT(emm_ctx, ue_mm_context_t, emm_context))->mme_ue_s1ap_id);
      } else {
        OAILOG_ERROR (LOG_NAS_EMM, "Could not stop T3460 stopped UE " MME_UE_S1AP_ID_FMT " ", (PARENT_STRUCT(emm_ctx, ue_mm_context_t, emm_context))->mme_ue_s1ap_id);
      }
    }
    ...
    if (emm_ctx->T3470.id != NAS_TIMER_INACTIVE_ID) {
      ...
      if (NAS_TIMER_INACTIVE_ID == emm_ctx->T3470.id) {
        MSC_LOG_EVENT (MSC_NAS_EMM_MME, "0 T3470 stopped UE " MME_UE_S1AP_ID_FMT " ", (PARENT_STRUCT(emm_ctx, ue_mm_context_t, emm_context))->mme_ue_s1ap_id);
        OAILOG_DEBUG (LOG_NAS_EMM, "T3470 stopped UE " MME_UE_S1AP_ID_FMT " ", (PARENT_STRUCT(emm_ctx, ue_mm_context_t, emm_context))->mme_ue_s1ap_id);
      } else {
        OAILOG_ERROR (LOG_NAS_EMM, "Could not stop T3470 stopped UE " MME_UE_S1AP_ID_FMT " ", (PARENT_STRUCT(emm_ctx, ue_mm_context_t, emm_context))->mme_ue_s1ap_id);
      }
    }
  }
}
  ...
  bstring bstr_dump)
{
  if (emm_context ) {
    ...
    bformata (bstr_dump, "%*s - EMM-CTX: ue id:           " MME_UE_S1AP_ID_FMT " (UE identifier)\n", indent_spaces, " ", (PARENT_STRUCT(emm_context, ue_mm_context_t, emm_context))->mme_ue_s1ap_id);
  ...
  void**resultP)
{
  ...
  emm_context_dump (static_cast<const emm_context_s*>(dataP), 4, bstr_dump);
