  ...
  bool is_ciphered)
{
  ...
  if (context && ((context->sc_type == SECURITY_CTX_TYPE_FULL_NATIVE) || (context->sc_type == SECURITY_CTX_TYPE_MAPPED))) {
    ...
    OAILOG_STREAM_HEX (OAILOG_LEVEL_DEBUG, LOG_NAS_EMM, "knas_int:",  reinterpret_cast<const char*>(context->knas_int), AUTH_KNAS_INT_SIZE);
    OAILOG_STREAM_HEX (OAILOG_LEVEL_DEBUG, LOG_NAS_EMM, "knas_enc:",  reinterpret_cast<const char*>(context->knas_enc), AUTH_KNAS_ENC_SIZE);
    data->is_new = is_new;
