...
void
dump_traffic_flow_template_xml (
  TrafficFlowTemplate * trafficflowtemplate,
  uint8_t iei)
{
  ...
  OAILOG_DEBUG (LOG_NAS, "    <Number of packet filters>%u</Number of packet filters>\n", trafficflowtemplate->numberofpacketfilters);
...
static int
decode_traffic_flow_template_delete_packet (
  DeletePacketFilter * packetfilter,
  uint8_t nbpacketfilters,
  uint8_t * buffer,
  uint32_t len)
...
static int
decode_traffic_flow_template_create_tft (
  CreateNewTft * packetfilter,
  uint8_t nbpacketfilters,
  uint8_t * buffer,
  uint32_t len)
...
static int
decode_traffic_flow_template_add_packet (
  AddPacketFilter * packetfilter,
  uint8_t nbpacketfilters,
  uint8_t * buffer,
  uint32_t len)
...
static int
decode_traffic_flow_template_replace_packet (
  ReplacePacketFilter * packetfilter,
  uint8_t nbpacketfilters,
  uint8_t * buffer,
  uint32_t len)
...
static int
decode_traffic_flow_template_packet_filter_identifiers (
  PacketFilterIdentifiers * packetfilteridentifiers,
  uint8_t nbpacketfilters,
  uint8_t * buffer,
  uint32_t len)
...
static int
decode_traffic_flow_template_packet_filters (
  PacketFilters * packetfilters,
  uint8_t nbpacketfilters,
  uint8_t * buffer,
  uint32_t len)
...
static int
encode_traffic_flow_template_delete_packet (
  DeletePacketFilter * packetfilter,
  uint8_t nbpacketfilters,
  uint8_t * buffer,
  uint32_t len)
...
static int
encode_traffic_flow_template_create_tft (
  CreateNewTft * packetfilter,
  uint8_t nbpacketfilters,
  uint8_t * buffer,
  uint32_t len)
...
static int
encode_traffic_flow_template_add_packet (
  AddPacketFilter * packetfilter,
  uint8_t nbpacketfilters,
  uint8_t * buffer,
  uint32_t len)
...
static int
encode_traffic_flow_template_replace_packet (
  ReplacePacketFilter * packetfilter,
  uint8_t nbpacketfilters,
  uint8_t * buffer,
  uint32_t len)
...
static int
encode_traffic_flow_template_packet_filter_identifiers (
  PacketFilterIdentifiers * packetfilteridentifiers,
  uint8_t nbpacketfilters,
  uint8_t * buffer,
  uint32_t len)
...
static int
encode_traffic_flow_template_packet_filters (
  PacketFilters * packetfilters,
  uint8_t nbpacketfilters,
  uint8_t * buffer,
  uint32_t len)
  ...
  uint8_t nbpacketfilters)
{
  ...
  for (i = 0; i < nbpacketfilters; i++) {
    ...
    if ((*packetfilters)[i].packetfilter.flags & TRAFFIC_FLOW_TEMPLATE_IPV6_REMOTE_ADDR_FLAG) {
      OAILOG_DEBUG (LOG_NAS, "            <Ipv6 remote address>%x%.2x:%x%.2x:%x%.2x:%x%.2x:%x%.2x:%x%.2x:%x%.2x:%x%.2x</Ipv6 remote address>\n", (*packetfilters)[i].packetfilter.ipv6remoteaddr[0].addr,
      ...
      OAILOG_DEBUG (LOG_NAS, "            <Ipv6 remote address mask>%x%.2x:%x%.2x:%x%.2x:%x%.2x:%x%.2x:%x%.2x:%x%.2x:%x%.2x</Ipv6 remote address mask>\n", (*packetfilters)[i].packetfilter.ipv6remoteaddr[0].mask,
