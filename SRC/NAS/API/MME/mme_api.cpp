  ...
  struct mme_config_s * mme_config_p)
{
  ...
  if (mme_config_p->unauthenticated_imsi_supported != 0) {
    config->features = static_cast<mme_api_feature_t>(config->features |MME_API_UNAUTHENTICATED_IMSI);
  }

  for (int i = 0; i < 8; i++) {
    config->prefered_integrity_algorithm[i] = nas_config.prefered_integrity_algorithm[i];
    config->prefered_ciphering_algorithm[i] = nas_config.prefered_ciphering_algorithm[i];
  }
  OAILOG_FUNC_RETURN (LOG_NAS, RETURNok);
}

// TODO
ue_mm_context_t * mme_api_duplicate_enb_ue_s1ap_id_detected (
  ...
  mme_api_esm_config_t * config)
{
  ...
  if (_mme_api_ip_capability == MME_API_IPV4_ADDR) {
    ...
    config->features = static_cast<mme_api_feature_t>(MME_API_IPV4 | MME_API_IPV6);
  } else {
    config->features = static_cast<mme_api_feature_t>(0);
  }
