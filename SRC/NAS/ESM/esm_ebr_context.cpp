  ...
  const protocol_configuration_options_t * pco)
{
  ...
  if (bidx < BEARERS_PER_UE) {
    ...
    if (bearer_context ) {
      ...
      bearer_context->esm_ebr_context.tft = const_cast<traffic_flow_template_t*>(tft);

      if (bearer_context->esm_ebr_context.pco) {
        free_protocol_configuration_options(&bearer_context->esm_ebr_context.pco);
      }
      bearer_context->esm_ebr_context.pco = const_cast<protocol_configuration_options_t*>(pco);
...
#warning "free pdn->bearer"
       ...
       */
#warning "Reset the PDN connection activation indicator"
      // .is_active = false;
