  ...
  const bool is_emergency)
{
  ...
  if (!ue_mm_context->pdn_contexts[pdn_cid]) {
    ...
    if (pdn_context ) {
      ...
      if (pco) {
        if (!pdn_context->pco) {
          pdn_context->pco = static_cast<protocol_configuration_options_t*>(calloc(1, sizeof(protocol_configuration_options_t)));
        } else {
      ...
      if (pdn_addr) {
        pdn_context->paa.pdn_type =  static_cast<pdn_type_value_t>(pdn_type);
        switch (pdn_type) {
        case IPv4:
