  ...
  esm_cause_t * const esm_cause)
{
  ...
  int                                     rc = RETURNerror;
  ue_mm_context_t                        *ue_mm_context = PARENT_STRUCT(ue_context, ue_mm_context_t, emm_context);
  ...
  const bool ue_triggered)
{
  OAILOG_FUNC_IN (LOG_NAS_ESM);
  int                                     rc;
  mme_ue_s1ap_id_t      ue_id = PARENT_STRUCT(ue_context, ue_mm_context_t, emm_context)->mme_ue_s1ap_id;
  ...
  STOLEN_REF bstring *msg)
{
  ...
  *msg = NULL;
  ...
  if (rc != RETURNerror) {
     ...
     */
    rc = esm_ebr_start_timer (ue_context, ebi, msg_dup, nas_config.t3495_sec, _eps_bearer_deactivate_t3495_handler);
  }else {
