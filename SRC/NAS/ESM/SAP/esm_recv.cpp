  ...
  ebi_t *new_ebi)
{
  ...
  if (msg->presencemask & PDN_CONNECTIVITY_REQUEST_ESM_INFORMATION_TRANSFER_FLAG_PRESENT) {
     ...
     */
    if (!nas_config.disable_esm_information) {
  ...
  nas_itti_pdn_config_req(pti, ue_id, &emm_context->_imsi, esm_data, static_cast<esm_proc_pdn_request_t>(esm_data->request_type));
  ...
  const esm_information_response_msg * msg)
{
  ...
  if (pid != RETURNerror) {
    ...
    nas_itti_pdn_config_req(pti, ue_id, &emm_context->_imsi, emm_context->esm_ctx.esm_proc_data, static_cast<esm_proc_pdn_request_t>(emm_context->esm_ctx.esm_proc_data->request_type));
