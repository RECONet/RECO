...
inline int
s6a_up_loc::s6a_parse_subscriber_status(
...
inline int
s6a_up_loc::s6a_parse_network_access_mode(
...
inline int
s6a_up_loc::s6a_parse_access_restriction_data(
...
inline int
s6a_up_loc::s6a_parse_ambr(
  struct avp *avp_ambr,
  ambr_t * ambr) {
  struct avp                             *avp = NULL;
  struct avp_hdr                         *hdr;

  CHECK_FCT(fd_msg_browse(avp_ambr, MSG_BRW_FIRST_CHILD, \
            (msg_or_avp**)&avp, NULL));
  ...
  while (avp) {
     ...
     */
    CHECK_FCT(fd_msg_browse(avp, MSG_BRW_NEXT, (msg_or_avp**)&avp, NULL));
  }
...
inline int
s6a_up_loc::s6a_parse_all_apn_conf_inc_ind(
  struct avp_hdr *hdr,
...
inline int
s6a_up_loc::s6a_parse_service_selection(
...
inline int
s6a_up_loc::s6a_parse_priority_level(
  struct avp_hdr *hdr,
...
inline int
s6a_up_loc::s6a_parse_pre_emp_capability(
  struct avp_hdr *hdr,
...
inline int
s6a_up_loc::s6a_parse_pre_emp_vulnerability(
  struct avp_hdr *hdr,
...
inline int
s6a_up_loc::s6a_parse_allocation_retention_priority(
  struct avp *avp_arp,
   ...
   */
  ptr->pre_emp_capability = (pre_emption_capability_t)\
   ...
   */
  ptr->pre_emp_vulnerability = (pre_emption_vulnerability_t)\
                               PRE_EMPTION_VULNERABILITY_ENABLED;
  CHECK_FCT(fd_msg_browse(avp_arp, MSG_BRW_FIRST_CHILD, \
            (msg_or_avp**)&avp, NULL));
  ...
  while (avp) {
    ...
    case AVP_CODE_PRE_EMPTION_CAPABILITY:
      CHECK_FCT(s6a_parse_pre_emp_capability(hdr, \
                &ptr->pre_emp_capability));
      break;

    case AVP_CODE_PRE_EMPTION_VULNERABILITY:
      CHECK_FCT(s6a_parse_pre_emp_vulnerability(hdr, \
...
inline int
s6a_up_loc::s6a_parse_eps_subscribed_qos_profile(
  struct avp *avp_qos,
  eps_subscribed_qos_profile_t * ptr) {
  struct avp                             *avp = NULL;
  struct avp_hdr                         *hdr;

  CHECK_FCT(fd_msg_browse(avp_qos, MSG_BRW_FIRST_CHILD, \
            (msg_or_avp**)&avp, NULL));
  ...
  while (avp) {
    ...
    case AVP_CODE_ALLOCATION_RETENTION_PRIORITY:
      CHECK_FCT(s6a_parse_allocation_retention_priority(avp, \
                &ptr->allocation_retention_priority));
      break;
...
inline int
s6a_up_loc::s6a_parse_ip_address(
  struct avp_hdr *hdr,
  ...
  if (ip_type == IANA_IPV4) {
    ...
    DevCheck(hdr->avp_value->os.len == 18, \
             hdr->avp_value->os.len, 18, ip_type);
...
inline int
s6a_up_loc::s6a_parse_apn_configuration(
  struct avp *avp_apn_conf_prof,
  apn_configuration_t * apn_config) {
  struct avp                             *avp = NULL;
  struct avp_hdr                         *hdr;

  CHECK_FCT(fd_msg_browse(avp_apn_conf_prof, \
            MSG_BRW_FIRST_CHILD, (msg_or_avp**)&avp, NULL));
  ...
  while (avp) {
    ...
    case AVP_CODE_SERVED_PARTY_IP_ADDRESS:
      ...
      CHECK_FCT(s6a_parse_ip_address(hdr, \
                &apn_config->ip_address[apn_config->nb_ip_address]));
    ...
    case AVP_CODE_SERVICE_SELECTION:
      CHECK_FCT(s6a_parse_service_selection(hdr, \
                 apn_config->service_selection, \
                 &apn_config->service_selection_length));
      break;

    case AVP_CODE_EPS_SUBSCRIBED_QOS_PROFILE:
      CHECK_FCT(s6a_parse_eps_subscribed_qos_profile(avp, \
                &apn_config->subscribed_qos));
      break;
...
inline int
s6a_up_loc::s6a_parse_apn_configuration_profile(
  struct avp *avp_apn_conf_prof,
  ...
  while (avp) {
    ...
    case AVP_CODE_ALL_APN_CONFIG_INC_IND:
      CHECK_FCT(s6a_parse_all_apn_conf_inc_ind(hdr, \
                &apn_config_profile->all_apn_conf_ind));
      break;
    ...
    case AVP_CODE_APN_CONFIGURATION: {
        ...
        CHECK_FCT(s6a_parse_apn_configuration(avp, \
                   &apn_config_profile->apn_configuration\
...
int
s6a_up_loc::s6a_parse_subscription_data(
  struct avp *avp_subscription_data,
  subscription_data_t * subscription_data) {
  struct avp                             *avp = NULL;
  struct avp_hdr                         *hdr;

  CHECK_FCT(fd_msg_browse(avp_subscription_data, \
             MSG_BRW_FIRST_CHILD, (msg_or_avp**)&avp, NULL));
  ...
  while (avp) {
    ...
    case AVP_CODE_SUBSCRIBER_STATUS:
      CHECK_FCT(s6a_parse_subscriber_status(hdr, \
    ...
    case AVP_CODE_NETWORK_ACCESS_MODE:
      CHECK_FCT(s6a_parse_network_access_mode(hdr, \
                &subscription_data->access_mode));
      break;

    case AVP_CODE_ACCESS_RESTRICTION_DATA:
      CHECK_FCT(s6a_parse_access_restriction_data(hdr, \
                &subscription_data->access_restriction));
      break;
    ...
    case AVP_CODE_APN_CONFIGURATION_PROFILE:
      CHECK_FCT(s6a_parse_apn_configuration_profile(avp, \
                &subscription_data->apn_config_profile));
      break;
