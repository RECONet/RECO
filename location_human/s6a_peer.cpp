...
*/
  ...
  strcat(host_name, "openair4G.eur");
  ...
  bstring  hss_name = bstrcpy(s6a_config.hss_host_name);
  bconchar(hss_name, '.');
  bcatcstr(hss_name, "openair4G.eur");
...
/*
          ...
          if (RUN_MODE_SCENARIO_PLAYER == mme_config.run_mode) {
            message_p = itti_alloc_new_message(TASK_S6A, ACTIVATE_MESSAGE);
            itti_send_msg_to_task(TASK_MME_SCENARIO_PLAYER, \
